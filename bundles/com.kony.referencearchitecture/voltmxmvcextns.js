  ...
  BusinessController.prototype.registerCommandHandlers = function (commandHandlers, callback) {
    if (callback == '' || callback == undefined) {
      for (var i = 0; i < commandHandlers.length; i++) {
        ...
        var filePath = _voltmx.mvc.getPath(commandHandlers[i].CommandHandler, this.appName);
        var commandHandler = require(filePath);
        if (commandHandlers[i].CommandHandlerExtension && commandHandlers[i].CommandHandlerExtension != null) {
          ...
          try {
            var path = _voltmx.mvc.getPath(commandHandlerExtn, this.appName);
            var commandExtension = require(path);
          } catch (err) {
      ...
      var self = this;
      var handlersarray = [], commandId = [], commandHandlerExtn = [];
      for (var i = 0; i < commandHandlers.length; i++) {
          ...
          try {
            ...
            if (commandHandlerExtn[i] != null || commandHandlerExtn[i] != undefined) {
              var commandExtension = require([HandlerExtn], function (instanceCommandHandlerExtn) {
                ...
                if (instanceCommandHandlerExtn && instanceCommandHandlerExtn.execute)
                  instanceCommandHandlerExtn.sendResponse = handler.sendResponse;
              });
            } else {
              var commandExtension = null;
            }
...
DataModel_DBAssembler = function (Error) {
  ...
  assembler.fromDBJson = function (BaseModelClass, config, dbJson) {
    ...
    */
    ...
    if (retModel.fromJsonInternal)
      retModel.fromJsonInternal(modelJson);
    else
      retModel = new BaseModelClass(modelJson);
      ...
      if (model) {
        ...
        if (Array.isArray(attributeDBValue)) {
          ...
          for (var record in attributeDBValue) {
            ...
            if (attributeModel.fromJsonInternal)
              attributeModel.fromJsonInternal(modelAttributeJson);
            else
              attributeModel = new modelDefinition(modelAttributeJson);
...
commonUtils_Utils = function () {
  function Utils() {
  }
  Utils.isNullOrUndefined = function (parameter) {
...
DataModel_DataSource = function (QueryBuilder, constants, DBAssembler, Utils) {
  DataSource = function () {
  };
  /**
  ...
  DataSource.prototype.customVerb = function (BaseModel, config, customVerb, params, onCompletion, headerParams, dtoOptions) {
    ...
    var headers = {};
    var customVerbSettings;
    headers[constants.SESSIONTOKEN] = voltmx.mvc.MDAApplication.getSharedInstance().appContext.session_token;
    for (var headKey in headerParams) {
      headers[headKey] = headerParams[headKey];
    }
    if (!Utils.isNullOrUndefined(params) && !Utils.isNullOrUndefined(params['customVerbSettings'])) {
      customVerbSettings = params['customVerbSettings'];
      delete params['customVerbSettings'];
    }
    //Adds passthrough to options if provided by user else initializes it with empty json.
    var options = getPassthrough(dtoOptions);
    var dataObject = new voltmx.sdk.dto.DataObject(config.tableName);
    ...
    if (params != null && params != undefined && params instanceof voltmx.mvc.Data.BaseModel) {
      ...
      for (var key in params) {
        if (params[key] != null && params[key] != undefined) {
          ...
          if (descriptor != null && descriptor != undefined && descriptor['get'])
            params[key] = dbo[key];
        }
    ...
    if (xmlOptions)
      options[constants.XML_REQUEST_OPTIONS] = xmlOptions;
    var generateModel = function (response) {
    ...
    objSvc.customVerb(customVerb, options, function (response) {
      var sendRawResponse = false;
      if (!Utils.isNullOrUndefined(customVerbSettings) && !Utils.isNullOrUndefined(customVerbSettings['sendRawResponse'])) {
        sendRawResponse = customVerbSettings['sendRawResponse'];
      }
      /*if "sendRawResponse" is false, then decision whether to send model or response
      should be based on table name present in response*/
      if (!sendRawResponse) {
        if (response[config.tableName])
          generateModel(response);
        else
  ...
  */
  ...
  return DataSource;
}(DataModel_QueryBuilder, DataModel_constants, DataModel_DBAssembler, commonUtils_Utils);
...
DataModel_RepositoryManager = function (BaseRepository) {
  ...
  RepositoryManager.prototype.initializeRepoConfig = function (repoconfig) {
    try {
      ...
      console.log('RepoManagerConfig loading failed, RepoManagerConfig is generated only when Project has Object services.');
    }
  };
  /**
...
PresentationController_MDABasePresenter = function () {
  ...
  MDABasePresenter.prototype._createPresentationStackObject = function (presenter, formName, context) {
    ...
    stackObj.uniqueFormName = _voltmx.mvc.getNameSpacedValue(formName, presenter.appName);
    return stackObj;
  };
  MDABasePresenter.prototype._pushToPresentationStack = function (presenter, formName, context) {
    if (this._presentationStack.length != 0) {
      ...
      var uniqueFormName = _voltmx.mvc.getNameSpacedValue(formName, presenter.appName);
      if (topStackObj.uniqueFormName == uniqueFormName) {
...
BaseNavigator_MDABaseNavigator = function () {
  ...
  MDABaseNavigator.prototype.presentForm = function (form, uiTag, viewModel) {
    ...
    var formName = _voltmx.mvc.getNameSpacedValue(form, this.presentationController.appName);
    if (this._asyncNavStack.hasOwnProperty(formName)) {
      if (viewModel)
        this._asyncNavStack[formName].push(viewModel);
    } else {
      if (_currentForm && form == _currentForm.id) {
        var controller = _voltmx.mvc.GetController(form, true, { 'appName': this.presentationController.appName });
        controller.updateUI(context['viewModel']);
      } else {
        try {
          this._asyncNavStack[formName] = [];
...
ModuleManager_MDAModule = function (MDABasePresenter, MDABaseNavigator, BusinessController, constants, BusinessDelegator) {
  function MDAModule(moduleName, moduleMode, channel, callback, isStartup, info) {
    this.businessController = null;
    this.presentationController = null;
    this.navigator = null;
    ...
    try {
      var config = _voltmx.mvc.getNameSpacedValue(this.moduleName + 'Config', this.appName);
      this.moduleConfig = eval(config);
    ...
    if (this.moduleConfig === undefined) {
      ...
      if (this.moduleConfig.ModuleType === undefined && (businessControllerClass === undefined || businessControllerClass === '' || presentationControllerClass === undefined || presentationControllerClass === '')) {
        ...
        try {
          ...
          if (handlersarray)
            params = params.concat(handlersarray);
          if (commandHandlerExtn)
            params = params.concat(commandHandlerExtn);
          var DerivedController = require(params, function () {
            presentationControllerClass = arguments[0];
            businessControllerClass = arguments[1];
            if (presentationControllerClass)
            ...
            if (self.moduleConfig.ModuleType != undefined && self.moduleConfig.ModuleType == 'Manager')
              configureBusinessController();
            if (self.moduleConfig.ModuleType != undefined && self.moduleConfig.ModuleType == 'UIModule')
              self.createPresentationExtensionsAsync(Callback);
            if (self.moduleConfig.ModuleType === undefined)
              self.createPresentationExtensionsAsync(configureBusinessController);
          });
        } catch (err) {
  ...
  MDAModule.prototype.setUpBusinessController = function () {
    if (this.moduleConfig === undefined) {
      ...
      if (businessControllerClass === undefined || businessControllerClass === '') {
        ...
        try {
          var filePath = _voltmx.mvc.getPath(businessControllerClass, this.appName);
          var DerivedBusinessController = require(filePath);
    ...
    this.businessController.appName = this.appName;
  ...
  MDAModule.prototype.setUpPresentationController = function () {
    if (this.moduleConfig === undefined) {
      ...
      if (presentationControllerClass === undefined || presentationControllerClass === '') {
        ...
        try {
          var filePath = _voltmx.mvc.getPath(presentationControllerClass, this.appName);
          var DerivedPresentationController = require(filePath);
    ...
    this.presentationController.appName = this.appName;
    this.presentationController.formFriendlyName = this.moduleName;
    ...
    if (this.moduleConfig !== undefined) {
      ...
      if (presentationExtensions !== undefined) {
        ...
        for (var i = 0; i < presentationExtensions.length; i++) {
          try {
            extensions = _voltmx.mvc.getPath(presentationExtensions[i], this.presentationController.appName);
            extNModule.push(require(extensions));
          } catch (err) {
            throw new Exception('ERROR_CODE_300', 'Presentation Controller Extension invalid ' + err);
          }
          voltmx.print('MDA2**** extNModule : ' + extNModule);
    ...
    if (this.moduleConfig !== undefined) {
      ...
      if (businessExtensions !== undefined) {
        ...
        for (var i = 0; i < businessExtensions.length; i++) {
          try {
            extensions = _voltmx.mvc.getPath(businessExtensions[i], this.businessController.appName);
            extNModule.push(require(extensions));
          } catch (err) {
            throw new Exception('ERROR_CODE_300', 'Business Controller Extension invalid ' + err);
          }
...
ModuleManager_MDAModuleManager = function (MDAModule) {
  ...
  MDAModuleManager.prototype.loadModule = function (moduleName, moduleMode, channel, callback, info) {
    if (moduleMode == null)
      moduleMode = 'Default';
    voltmx.print('MDA2***..loadModule: ' + moduleName + '-' + moduleMode + 'Mode');
    var self = this;
    var appName = info.appName;
    var nameSpacedModuleName = _voltmx.mvc.getNameSpacedValue(this.moduleName, appName);
    if (!this.moduleMap[nameSpacedModuleName]) {
      var module = null;
      var moduleConfig;
      try {
        var config = _voltmx.mvc.getNameSpacedValue(this.moduleName + 'Config', appName);
        moduleConfig = eval(config);
      } catch (e) {
      }
      if (moduleConfig !== undefined) {
        ...
        if (moduleClass !== undefined || moduleClass !== '') {
          ...
          module = new Module(moduleName, moduleMode, channel, callback, null, info);
          this.moduleMap[nameSpacedModuleName] = module;
        } else {
          if (callback !== undefined) {
            new MDAModule(moduleName, moduleMode, channel, function (module) {
              self.moduleMap[nameSpacedModuleName] = module;
              callback(module);
            }, null, info);
          } else {
            module = new MDAModule(moduleName, moduleMode, channel, callback, null, info);
            this.moduleMap[nameSpacedModuleName] = module;
          }
        }
      } else {
        var self = this;
        nameSpacedModuleName = _voltmx.mvc.getNameSpacedValue(moduleName, appName);
        if (callback !== undefined) {
          new MDAModule(moduleName, moduleMode, channel, function (module) {
            self.moduleMap[nameSpacedModuleName] = module;
            callback(module);
  ...
  MDAModuleManager.prototype.getModule = function (module, moduleMode, channel, callback) {
    ...
    if (typeof moduleName == 'object') {
      ...
      moduleMode = module.moduleMode;
      appName = module.appName;
      moduleName = module.moduleName;
    } else {
      appName = _voltmx.mvc.getCurrentAppName();
    }
    var nameSpacedModuleName = _voltmx.mvc.getNameSpacedValue(moduleName, appName);
    if (!this.moduleMap[nameSpacedModuleName]) {
      this.loadModule(moduleName, moduleMode, channel, callback, { 'appName': appName });
      return this.moduleMap[nameSpacedModuleName];
    } else {
      if (callback === undefined) {
        return this.moduleMap[nameSpacedModuleName];
      } else {
        callback(this.moduleMap[nameSpacedModuleName]);
      }
    }
  };
  MDAModuleManager.prototype.loadStartupModule = function (moduleName, moduleMode, channel, callback, info) {
    if (moduleMode == null)
      moduleMode = 'Default';
    ...
    if (!this.moduleMap[nameSpacedModuleName]) {
      ...
      if (moduleConfig !== undefined) {
        ...
        if (moduleClass !== undefined || moduleClass !== '') {
          var Module = require(moduleClass);
          module = new Module(moduleName, moduleMode, channel, callback, isStartup, info);
          this.moduleMap[nameSpacedModuleName] = module;
        } else {
          if (callback !== undefined) {
            new MDAModule(moduleName, moduleMode, channel, function (module) {
            ...
            module = new MDAModule(moduleName, moduleMode, channel, callback, isStartup, info);
            this.moduleMap[nameSpacedModuleName] = module;
          }
        }
      } else {
  ...
  MDAModuleManager.prototype.getStartupModule = function (moduleName, moduleMode, channel, callback, info) {
    var appName = info.appName;
    var nameSpacedModuleName = _voltmx.mvc.getNameSpacedValue(moduleName, appName);
    if (!this.moduleMap[nameSpacedModuleName]) {
      this.loadModule(moduleName, moduleMode, channel, callback, info);
      return this.moduleMap[nameSpacedModuleName];
    } else {
      if (callback === undefined) {
        return this.moduleMap[nameSpacedModuleName];
...
commonUtils_ProcessorUtils = function () {
  ...
  ProcessorUtils.getMetadataForObject = function (serviceName, objectName, options, successCallback, failureCallback) {
    voltmx.sdk.getCurrentInstance().getObjectService(serviceName, { 'access': 'online' }).getMetadataOfObject(objectName, options, successCallback, failureCallback);
  };
...
FormController_MDAFormController = function (inheritsFrom) {
  ...
  MDAFormController.prototype.onNavigate = function (context, isBackNavigation) {
    ...
    if (context && context._presenter) {
      ...
      var controller = _voltmx.mvc.getControllerFromViewID(formId, appName);
      var formName = _voltmx.mvc.getNameSpacedValue(controller.formFriendlyName, appName);
