    ...
    var module = {
        ...
        computeSnapWidths: function(formId, widgetType, initialize) {
            ...
            for(var i = 0; i < swipeElements.length; i++) {
                ...
                if(widgetType == "Tabpane") {
                    widgetModel = $KU.getModelByNode(swipeElement.parentNode);
                }
        ...
        computeWidths: function(swipeElement, widgetModel) {
            ...
            if(slides.length > 0) {
                for(var j = 0; j < slides.length; j++) {
                    ...
                    if(widgetModel.wType == "Segment" || widgetModel.wType == "TabPane") {
                        slides[j].style.width = percent + "%"; 
                        if(widgetModel.wType == "TabPane") {
                            if($KU.isIE)
                                slides[j].style.styleFloat = "left";
                            else
                                slides[j].style.cssFloat = "left";
                        }
                    } else {
            ...
            if(widgetModel.wType == "TabPane") {
                ...
                widgetModel.swipeContext.currentPage = swipeContext.currentPage;
            }
        ...
        previousImage: function(touchContainerElement, swipeContext, orientationChanged) {
            ...
            if(swipeContext.wType == "TabPane") {
                var tabPaneModel = $KU.getModelByNode(imgsElement.parentNode.parentNode);
                tabPaneModel.activetab = Math.max(tabPaneModel.activetab - 1, 0);
                tabPaneModel.activetabs[IndexJL] = tabPaneModel.activetab;

                var tabID = tabPaneModel.children[tabPaneModel.activetab];
                $KW.TabPane.changeActiveTabSkin(tabPaneModel, tabID);
                module.updatePageIndicator(touchContainerElement.parentNode, swipeContext, tabPaneModel);
            }
        },
        ...
        nextImage: function(touchContainerElement, swipeContext, orientationChanged) {
            ...
            if(swipeContext.wType == "TabPane") {
                var tabPaneModel = $KU.getModelByNode(elem.parentNode.parentNode);
                tabPaneModel.activetab = Math.min(tabPaneModel.activetab + 1, noOfSwipePages - 1);
                tabPaneModel.activetabs[IndexJL] = tabPaneModel.activetab;
        ...
        scrollImages: function(elem, distance, duration, isOriented, isTabContext) {
            ...
            if(voltmx.appinit.useTransition)
                ...
                $("#" + elem.parentNode.id + " div:first").animate({
                    left: value + 'px'
                }, duration);
            }

            if(isTabContext && isTabContext.tabPaneModel && isTabContext.tabID) {
                $KW.TabPane.changeActiveTabSkin(isTabContext.tabPaneModel, isTabContext.tabID);
            }
        ...
        pageviewScroller: function(element, options) {
            ...
            if('ontouchstart' in window)
                ...
                voltmx.events.addEventListener(element, "mousedown", this.handleEventListener);
            }
        ...
        voltmxScroller: function(el, options) {
            ...
            if(that.options.useTransform) {
                ...
                if(voltmx.appinit.isFirefox || voltmx.appinit.isOpera) {
                    that.scroller.style.cssText += ';position:relative;top:' + (that.y - (that.options.vScroll ? that.pullDownOffset : 0)) + 'px;left:' + (that.x - (that.options.hScroll ? that.pullDownOffset : 0)) + 'px';
                } else {
            ...
            that.handleEventListener = that.handleEvent.bind(that);
            ...
            if(!that.options.disableUserScroll) {
                if($KU.isTouchSupported) 
                    ...
                    voltmx.events.addEventListener(that.scroller, "mousedown", that.handleEventListener);
                    voltmx.events.addEventListener(that.scroller, "mouseout", that.handleEventListener);
                }
            }
            ...
            if($KU.isOrientationSupported && !$KU.isAndroid) {
                ...
                voltmx.events.addEventListener(window, "resize", that.handleEventListener);
            }
        ...
        TouchEvents: function(widgetModel, widgetNode, eventType, callback) {
            ...
            if(callback || eventType == 'scroll' || $KAR) {
                ...
                if(eventType == "scroll") {
                    ...
                    voltmx.events.addEventListener(widgetNode, mouseEvent, this.handleEventListener, false);
    ...
    module.pageviewScroller.prototype = {
        ...
        onTouchStart: function(event) {
            ...
            if($KU.isTouchSupported) {
                ...
                voltmx.events.addEventListener(this.element, "mousemove", this.handleEventListener);
                voltmx.events.addEventListener(this.element, "mouseup", this.handleEventListener);
                voltmx.events.addEventListener(this.element, "mouseout", this.handleEventListener);
            }

            
            this.x = this.lastx = this.startX = touch.pageX || touch.clientX;
            this.y = this.lasty = this.startY = touch.pageY || touch.clientY;
        ...
        onTouchMove: function(event) {
            var touch = event.touches && event.touches[0] || event;
            var newX = touch.pageX || touch.clientX;
            var newY = touch.pageY || touch.clientY;
        ...
        detachEvents: function() {
            ...
            if($KU.isTouchSupported) {
                ...
                voltmx.events.removeEventListener(this.element, "mousemove", this.handleEventListener);
                voltmx.events.removeEventListener(this.element, "mouseup", this.handleEventListener);
                voltmx.events.removeEventListener(this.element, "mouseout", this.handleEventListener);
            }
        },
        ...
        destroy: function() {
            ...
            if(this.element) {
                if($KU.isTouchSupported) {
                    ...
                    voltmx.events.removeEventListener(this.element, "mousedown", this.handleEventListener);
                    voltmx.events.removeEventListener(this.element, "mouseout", this.handleEventListener);
                }
            }
        }
    ...
    module.voltmxScroller.prototype = {
        ...
        extendTouchStart: function(touch) {
            ...
            if(this.options.useTransform) {
                ...
                x = $KU.getStyle(this.scroller, "left").replace(/[^0-9-]/g, '') * 1;
                y = $KU.getStyle(this.scroller, "top").replace(/[^0-9-]/g, '') * 1;
            }
            ...
            this.pointX = touch.pageX || touch.clientX;
            this.pointY = touch.pageY || touch.clientY;
            ...
            if($KU.isTouchSupported) {
                ...
                voltmx.events.addEventListener(this.scroller, "mousemove", this.handleEventListener);
                voltmx.events.addEventListener(this.scroller, "mouseup", this.handleEventListener);
            }
        ...
        extendTouchMove: function(touch, event) {
            event = event || touch;
            var deltaX = (touch.pageX || touch.clientX) - this.pointX,
                deltaY = (touch.pageY || touch.clientY) - this.pointY,
            newX = this.x + deltaX,
                newY = this.y + deltaY,
                c1, c2,
        ...
        extendTouchEnd: function(touch) {
            ...
            if($KU.isTouchSupported) {
                ...
                voltmx.events.removeEventListener(this.scroller, "mousemove", this.handleEventListener);
                voltmx.events.removeEventListener(this.scroller, "mouseup", this.handleEventListener);
            }
