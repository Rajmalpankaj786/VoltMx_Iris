...
$KI.media = {
    record: function(config) {
        var _audioRecorder = null, _record = null;
        if(arguments.length === 2) {
            config = arguments[1];
        }
        $KU.logExecuting('voltmx.media.record');
                ...
                startRecording: function() {
                    if(!_audioRecorder || (_audioRecorder && _audioRecorder.state === 'inactive')) {
                        navigator.mediaDevices.getUserMedia({audio:true}).then(function(stream) {
                            ...
                            voltmx.web.logger('info', 'recording audio');
                        }).catch( function(err) {
        ...
        $KU.logExecutingFinished('voltmx.media.record');
        return _record;
    }
}
