    ...
    var module = {
        ...
        setDimensions: function(wModel, wNode, flexNode) {
            if(wNode) {
                ...
                else if(wModel.wType == "ListBox" && wModel.viewtype == constants.LISTBOX_VIEW_TYPE_EDITVIEW) {
                    $KW.ListBox.adjustHeight(wNode);
                    $KW.ListBox.adjustWidth(wModel, wNode);
                }
            }
        },
        ...
        calculateSpan: function(widgetModel, resConfig) {
            var span, spanConfig = null, i, curBrkPointIndex;
            var curBrkPoint = $KG["__currentFormBreakpoint"];
            var brkPoints = $KG["__currentForm"].breakpoints.slice();

            if(resConfig && resConfig.span) {
                ...
                for(i = curBrkPointIndex; i >= 0; i--) {
                    if(typeof spanConfig[brkPoints[i]] === 'number') {
        ...
        calculateOffset: function(widgetModel, resConfig) {
            var offset, offsetConfig = null, i;
            var curBrkPoint = $KG["__currentFormBreakpoint"];
            var brkPoints = $KG["__currentForm"].breakpoints.slice();
            brkPoints.push(constants.BREAKPOINT_MAX_VALUE);
            var curBrkPointIndex = brkPoints.indexOf(curBrkPoint);

            if(resConfig && resConfig.offset)  {
                ...
                for(i = curBrkPointIndex; i >= 0; i--) {
                    if(typeof offsetConfig[brkPoints[i]] === 'number') {
            ...
            if(offset === undefined || offset > 12 || offset < 0) {
                 offset = 0
            }
