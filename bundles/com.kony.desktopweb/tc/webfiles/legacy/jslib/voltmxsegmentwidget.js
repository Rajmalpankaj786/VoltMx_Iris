...
$KW.Segment = (function() {
    ...
    var module = {
        ...
        initializeView: function(formId) {
            ...
            this.initializeAnimations(formId);
            var scrollNodes = document.querySelectorAll("#" + formId + " div[kwidgettype='Segment']");
            $KW.Utils.initializeScrollEvents(scrollNodes);
            $KW.APIUtils.getModelForContentOffset(formId, "Segment");
        },

        initializeAnimations: function(formId) {
            var segments = document.querySelectorAll("#" + formId + " div[kwidgettype='Segment']");
            for(var i = 0; i < segments.length; i++) {
                var segModel = $KU.getModelByNode(segments[i]);
                var listAnimation = segModel && segModel.listAnimation;
        ...
        destroyAnimations: function(formId) {
            var segments = document.querySelectorAll("#" + formId + " div[kwidgettype='Segment']");
            for(var i = 0; i < segments.length; i++) {
                var segModel = $KU.getModelByNode(segments[i]);
                var listAnimation = segModel && segModel.listAnimation;
            ...
            switch(propertyName) {
                ...
                case "viewtype":
                    ...
                    if(propertyValue != oldPropertyValue && element) {
                        ...
                        $KU.setScrollHeight(widgetModel, element);
                    }
                    break;
        ...
        generateSegment: function(segmentModel, context) {
            ...
            var skin = segmentModel.skin;
            
            var widgetSkin = (segmentModel.name == "voltmx.ui.SegmentedUI2") ? (segmentModel.widgetskin ? segmentModel.widgetskin : "") : (skin ? skin : "");
            var computedSkin = visibility + " " + widgetSkin;
            ...
            if(pageView) {
                ...
                htmlString += "<div id='" + id + "_wrapper' style='" + margin + ";position:relative' class='" + " " + computedSkin + "' " + kmasterObj.kmasterid + ">";
                ...
                htmlString += "<div" + $KW.Utils.getBaseHtml(segmentModel, context);
                htmlString += "class='" + computedSkin + "' style='" + margin + "'>";
            }

            if(segData.length > IndexJL) {
                if(!pageView) segmentModel.hasSections = $KU.isArray(segData[IndexJL]);
        ...
        modifyContent: function(model, data, action, rowIndex, secIndex, animObj) {
            ...
            if(!isFlexWidget)
                $KU.toggleVisibilty(segmentNode, model.data, model);
            ...
            if(data && data.length > 0) {
                ...
                if(module.isLazyLoadingApplicable(segmentModel)) {
                    ...
                    || (segmentModel.hasSections && segNode.children.length == 0)) {
                        module.registerScrollEvents(segmentModel);
                        ...
                        if(!segmentModel.previousHeight) {
                            module.registerScrollEvents(segmentModel);
                        }
        ...
        applyWidgetFocusSkin: function(widgetData, childModel, segmentModel) {
            ...
            if(widgetData && widgetData.hoverSkin) {
                var classSelector = "";
                if(segmentModel.hasSections) {
                    classSelector = "#" + segID + " li[secindex='" + segmentModel.seccounter + "," + segmentModel.rowcounter + "'] #" + segChildID + ":hover";
                } else {
                    classSelector = "#" + segID + " li[index='" + segmentModel.rowcounter + "'] #" + segChildID + ":hover";

                }
                $KW.skins.applyStyle(widgetData.hoverSkin, classSelector, segmentModel.wType);
            }
        },
    ...
    registerScrollEvents: function(segmentModel) {
        segmentModel.enableScrolling = true;
        segmentModel.scrolldirection = voltmx.flex.SCROLL_VERTICAL;
        segmentModel.onScrollStart = module.handleLazyLoadingOnScrollStart;
        segmentModel.onScrolling = module.handleLazyLoadingOnScrollMove;
        segmentModel.onScrollEnd = module.handleLazyLoadingOnScrollEnd;
    },
